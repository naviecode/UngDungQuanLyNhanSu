...
class Timesheet(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Create a parent frame that will center the calendar
        ...
        # Create a frame for month/year selection
        ...
        self.month_combobox['values'] = [i for i in range(1, 13)]  # Month values from 1 to 12
        ...
        self.month_combobox.current(today.month - 1)  # month index starts from 0
        self.year_combobox.current(5)  # center around current year

        # Add a Canvas widget and Scrollbar for the calendar frame
        ...
        # Scroll to the top of the canvas after the content is created
        self.canvas.after(50, lambda: self.canvas.yview_moveto(0))

    def on_month_year_change(self, event):
        month_index = self.month_combobox.get()  # Get selected month
        year = int(self.year_combobox.get())  # Get selected year
    ...
    def update_calendar(self, month, year):
        ...
        # Create a new calendar in the frame
        create_calendar(self.frame_calendar, month, year)

def create_calendar(parent, month, year):
    ...
    cal = calendar.Calendar(firstweekday=0)  # First day of the week is Monday
    ...
    # Days in previous and next month
    prev_month_days = calendar.monthrange(prev_year, prev_month)[1]
    next_month_day = 1

    # Load the striped image and resize it
    ...
    # Iterate through the weeks (rows) in the month
    for week in month_days:
        for col, day in enumerate(week):
            if day == 0:  # From previous or next month
                if row_number == 1:  # First week (previous month days)
                    day = prev_month_days - week[:col].count(0)
                    display_date = f'{day}'
                else:  # Last week (next month days)
                ...
                hours_label.image = stripe_img  # Keep reference to prevent garbage collection

            else:  # Normal days within the current month
                day_frame = tk.Frame(parent, borderwidth=2, relief="solid", width=15, height=7)
                day_frame.grid(row=row_number, column=col, sticky="nsew")
